...
public class Toml4jConfigSerializer<T extends ConfigData> implements ConfigSerializer<T> {
    ...
    private Path getConfigPath() {
        Path path = Utils.getConfigFolder().resolve(definition.name() + ".toml").toAbsolutePath();
        IZMK.logger.debug("config path: " + path);
        return path;
    }
    ...
    public T deserialize() throws SerializationException {
        ...
        if (Files.exists(configPath)) {
            try {
                ...
                IZMK.logger.debug("Error while reading config file: " + e);
                T config = createDefault();
                serialize(config);
                IZMK.logger.debug("Created default config file");
                return config;
            }
        } else {
            IZMK.logger.warn("Config file not found, creating default config");
