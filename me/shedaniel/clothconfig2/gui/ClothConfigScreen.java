    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (isEditable()) {
            ...
            if (errors.size() > 0) {
                ...
                String text = "┬ºc" + (errors.size() == 1 ? errors.get(0).plainCopy().getString() : I18n.get("text.cloth-config.multi_error"));
                if (isTransparentBackground()) {
            ...
            RenderSystem.setShaderTexture(0, CONFIG_TEX);
            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
            String text = "┬ºc" + I18n.get("text.cloth-config.not_editable");
            if (isTransparentBackground()) {
                int stringWidth = minecraft.font.width(text);
                fillGradient(matrices, 8, 9, 20 + stringWidth, 14 + minecraft.font.lineHeight, 0x68000000, 0x68000000);
            }
